#!/usr/bin/env python3
"""
COMPASS — compass_3linee.py v2.1
══════════════════════════════════
...
Fase 2: rotation capitale tra le 3 linee basata su regime + drawdown A
...
YIELD_STIMATI_O = {
    ...
    "EUEB.MI": 3.3,  # iShares EUR Corp Bond Enhanced Active
}

O_CONF_MIN_REGIME   = 65
O_PERSIST_SETTIMANE = 3
O_FLOOR_MENSILE     = -3.0
O_CEDOLA_MIN_NETTA  = 3.0
...
LINEE_INFO = {
    ...
    "O": {
        ...
        "pool": [
            ...
            "EUEB.MI",  # iShares EUR Corp Bond Enhanced Active
        ],
        "colore": "#8B5CF6",
    },
    "A": {
        ...
        "pool": [
            ...
            "XUTC.MI",
            # ── iShares Enhanced Active ─────────────────────────────
            "WOEE.DE", "EMEE.MI", "USEE.DE", "EUEE.DE", "AXEE.MI",
            # ── AI Innovation (solo C9/A9, peso max 15%) ────────────
            "IART.DE",
        ],
        "colore": "#DC2626",
    },
...
# ── FASE 2 — COSTANTI ROTATION ─────────────────────────────────────────────
# M = 5% fisso salvo crisi. Rotation avviene solo tra O e A.
FASE2_ALLOC_NORMALE = {
...
# (s_min, s_max, livello, M%, O%, A%)  — None = usa FASE2_ALLOC_NORMALE
FASE2_SOGLIE_CRISI = [
...
# Rientro graduale dopo fine crisi (sett_da, sett_a, nome, M%, O%, A%)
FASE2_RIENTRO = [
...
FASE2_SOGLIA_RIBILANCIO_PCT = 3.0
...
ETF_CATEGORIA = {
    ...
    "IBCZ.DE":"az_globale","IS07.DE":"az_globale","FCRN.DE":"az_globale","IWQU.MI":"az_globale",
    "EXX5.DE":"az_europa","EXV1.DE":"az_europa","EXXW.DE":"az_europa",
    "IEMO.MI":"az_europa","IEQU.MI":"az_europa",
    "QDVB.DE":"az_usa","WRTY.MI":"az_usa",
    "IS3N.DE":"az_em","VAPX.MI":"az_em","JPNH.MI":"az_em","NTSZ.MI":"az_em",
    ...
    # ── iShares Enhanced Active ────────────────────────────────────────
    "WOEE.DE":"az_globale","EMEE.MI":"az_em","USEE.DE":"az_usa",
    "EUEE.DE":"az_europa","IART.DE":"tematico","AXEE.MI":"az_em",
    "EUEB.MI":"obbligaz_ig",
}
...
ETF_NOMI = {
    ...
    # ── iShares Enhanced Active + tematici ────────────────────────────
    "WOEE.DE":"iShares World Equity Enhanced Active",
    "EMEE.MI":"iShares Emerging Markets Equity Enhanced Active",
    "USEE.DE":"iShares US Equity Enhanced Active",
    "EUEE.DE":"iShares Europe Equity Enhanced Active",
    "IART.DE":"iShares AI Innovation Active",
    "AXEE.MI":"iShares Asia ex Japan Equity Enhanced Active",
    "EUEB.MI":"iShares EUR Corporate Bond Enhanced Active",
}
...
PERSISTENZA = {"settimane": 2, "soglia": 60}
...
ROTATION_THRESHOLD = {"M": 999, "O": 12.0, "A": 15.0}
...
def calcola_fase2(portafogli, regime_corrente, stato_precedente=None):
    ...
    # 2. drawdown corrente Linea A dal picco storico
    drawdown_A = 0.0
    ptf_A = portafogli.get("A", {})
    if ptf_A:
        cap_att = ptf_A.get("capitale_attuale", 0)
        storia  = ptf_A.get("storia", [])
        picco   = max((v.get("capitale_attuale", 0) for v in storia), default=cap_att)
    ...
    if crisi_prev and not crisi_now and not in_rientro:
        in_rientro = True; data_fine_crisi = oggi; step_rientro = 0

    if in_rientro and crisi_now:
        in_rientro = False; data_fine_crisi = None; step_rientro = 0
    ...
    if crisi_now:
        motivo = (f"Crisi {livello_crisi} — score {score_crisi:.0f} "
                  f"(regime {regime_crisis_pct:.0f}% + dd_A {drawdown_A:.1f}%)")
        ...
        for idx, (s_da, s_a, step_nome, m_r, o_r, a_r) in enumerate(FASE2_RIENTRO):
            if s_da <= settimane_rientro < s_a:
                ...
                if o_pct is None or a_pct is None:
                    ...
                    m_pct, o_pct, a_pct = alloc["M"], alloc["O"], alloc["A"]
                    in_rientro = False
    ...
    if m_pct is not None and o_pct is not None and a_pct is not None:
        ...
        if abs(tot - 100) > 0.5 and (o_pct + a_pct) > 0:
            f = (100.0 - m_pct) / (o_pct + a_pct)
    ...
    delta_M = round(cap_M_tgt - portafogli.get("M", {}).get("capitale_attuale", float(CAPITALE)), 2)
    delta_O = round(cap_O_tgt - portafogli.get("O", {}).get("capitale_attuale", float(CAPITALE)), 2)
    delta_A = round(cap_A_tgt - portafogli.get("A", {}).get("capitale_attuale", float(CAPITALE)), 2)

    ribilancio_necessario = any(
        abs(d) / cap_totale * 100 > FASE2_SOGLIA_RIBILANCIO_PCT
        for d in [delta_M, delta_O, delta_A] if cap_totale > 0
    )

    # 9. stato persistente
    stato_nuovo = {
        "data": oggi, "livello_crisi": livello_crisi, "score_crisi": score_crisi,
        "in_rientro": in_rientro,
        "data_fine_crisi": data_fine_crisi if in_rientro else None,
        "step_rientro": step_rientro, "settimane_rientro": settimane_rientro,
    }

    return {
        "data": oggi, "regime": regime_corrente,
        "livello_crisi": livello_crisi, "score_crisi": score_crisi,
        "regime_crisis_pct": regime_crisis_pct, "drawdown_A_pct": drawdown_A,
...
def calc_score(closes, linea_id, yield_pct=0):
    ...
    if linea_id == "M":
        ...
        score_stab = max(0, 50 - vol*10)
        score_yield = min(50, yield_pct * 10)
...
def seleziona_etf_O(regime, etf_data, target_date, capitale):
    ...
    for item in template:
        ...
        comp.append({
            ...
            "peso": float(peso), "importo": importo,
            "price_entry": round(price, 4),
            "quote": round(importo/price, 4) if price else 0,
            "yield_pct": yield_stimato, "score_entry": 0, "mom3m_entry": 0,
        })
...
def seleziona_etf(pool, linea_id, etf_data, n_max, target_date, regime=None, capitale=None):
    ...
    comp = []
    # ETF con peso massimo vincolato
    PESO_MAX_VINCOLATO = {"IART.DE": 15.0}
    for c in selected:
        p = round(c["score"]/tot*100, 1)
        # Applica cap peso se necessario
        if c["ticker"] in PESO_MAX_VINCOLATO:
            p = min(p, PESO_MAX_VINCOLATO[c["ticker"]])
        imp = round(capitale*p/100, 2)
        comp.append({
            "ticker": c["ticker"], "nome": ETF_NOMI.get(c["ticker"], c["ticker"]),
            "peso": p, "importo": imp,
            "price_entry": round(c["price"], 4),
            "quote": round(imp/c["price"], 4) if c["price"] else 0,
            "yield_pct": c["yield_pct"], "score_entry": c["score"],
...
def check_rotation(composizione, pool, etf_data, linea_id, target_date,
    ...
    return {
        ...
        "motivo": f"rotation: {min_ticker} mom3M {min_mom:.1f}% → {migliore} mom3M {migliore_mom:.1f}%"
    }
...
def run_backtest_O(etf_data, proxy_data):
    ...
    def get_price(ticker, date):
        ...
        for i in range(min(len(closes), len(dates))):
            if dates[i] <= date: best = closes[i]
            elif dates[i] > date: break
        return best
    ...
    def build_comp(regime, capital, date):
        ...
        for item in template:
            ...
            if not price: print(f"      ⚠ {t} N/D al {date}"); continue
            peso = item["peso"]
    ...
    if not comp_v1: print("      ❌ Impossibile costruire V1"); return None

    versioni = []; ver_num = 1
    equity_curve = [cap]; equity_dates = [BACKTEST_START]
    ...
    ver_attiva = {
        ...
        "_equity": [cap], "cedola_aggregata_pct": calc_cedola_agg(comp_v1),
    }
    versioni.append(ver_attiva)

    ultimo_check = BACKTEST_START; settimane_cand = 0; regime_cand = None
    cap_inizio_mese = cap; mese_corrente = BACKTEST_START[:7]
    rendimenti_mensili = {}
    ...
    for data_corrente in all_dates:
        ...
        for etf in ver_attiva["composizione"]:
            ...
            pe = etf["price_entry"]; y = etf.get("yield_pct", 0)
            tr = calc_total_return(pe, price_now, y, giorni)
            etf["price_now"] = round(price_now, 4); etf["perf_pct"] = round(tr, 3)
        ...
        equity_curve.append(cap_now); equity_dates.append(data_corrente)
        mese = data_corrente[:7]
        if mese != mese_corrente:
            if cap_inizio_mese > 0:
                rendimenti_mensili[mese_corrente] = round(
                    (cap_now - cap_inizio_mese) / cap_inizio_mese * 100, 2)
            cap_inizio_mese = cap_now; mese_corrente = mese
        try:
            giorni_check = (datetime.date.fromisoformat(data_corrente) -
                            datetime.date.fromisoformat(ultimo_check)).days
        except Exception:
            giorni_check = 0
        cambio_regime = None; floor_breach = False
        if giorni_check >= 5:
            ...
            if sc != regime_attivo and not floor_breach:
                if sc == regime_cand: settimane_cand += 1
                else: regime_cand = sc; settimane_cand = 1
        ...
        if cambio_regime or floor_breach:
            ...
            if not nuova_comp: continue
            acquistati = [{"ticker":e["ticker"],"nome":e.get("nome",e["ticker"]),
                           "peso":e["peso"],"importo":e["importo"],
                           "price_entry":e["price_entry"],"score_entry":0,"mom3m_entry":0.0}
                          for e in nuova_comp]
            ...
            ver_attiva = {
                ...
                "_equity": [cap_now], "cedola_aggregata_pct": calc_cedola_agg(nuova_comp),
                "_venduti": venduti, "_acquistati": acquistati,
            }
            versioni.append(ver_attiva)

    if mese_corrente and cap_inizio_mese > 0:
        rendimenti_mensili[mese_corrente] = round(
            (equity_curve[-1] - cap_inizio_mese) / cap_inizio_mese * 100, 2)

    diario = []
    for v in versioni[1:]:
        diario.append({
            "data": v["data_apertura"],
            "versione_da": v["versione"] - 1, "versione_a": v["versione"],
            "trigger": v["trigger_apertura"], "regime": v["regime"],
            "capitale_ribilanciamento": v["capitale_inizio"],
            "venduti": v.pop("_venduti", []), "acquistati": v.pop("_acquistati", []),
        })
    ...
    print(f"      → {len(versioni)} ver, {len(diario)} rib, perf {perf_fin:+.1f}%{vol_str}, cedola {cedola_fin:.1f}% {'✅' if cedola_ok else '❌'}")

    return {
        "storia": versioni, "performance_totale_pct": perf_fin,
        "performance_totale_eur": round(cap_fin - CAPITALE, 2),
        "capitale_attuale": cap_fin, "volatilita_ann": vol_tot,
        "max_drawdown": mdd_tot, "sharpe": sharpe_tot,
        "rendimenti_mensili": rendimenti_mensili, "diario_movimenti": diario,
        "versione_corrente": len(versioni), "benchmark": LINEE_INFO["O"]["benchmark"],
        "rotation_suggerita": None, "cedola_aggregata_pct": cedola_fin,
        "cedola_ok": cedola_ok, "outperformance_bm": None, "batte_benchmark": None,
    }

# ── BACKTEST LINEA M e A ───────────────────────────────────────────────────
def run_backtest_linea(linea_id, etf_data, proxy_data):
    info = LINEE_INFO[linea_id]
    pool = info["pool"]; n_max = info["n_etf_max"]
    ...
    versioni = []; ver_num = 1; cap = float(CAPITALE)
    equity_curve = [cap]; equity_dates = [BACKTEST_START]
    ver_attiva = {
        ...
        "performance_pct": 0.0, "performance_eur": 0.0,
        "giorni_attivo": 0, "composizione": comp_v1,
        "etf_usciti": [], "aggiornato": BACKTEST_START, "_equity": [cap],
    }
    versioni.append(ver_attiva)
    ultimo_check = BACKTEST_START; regime_attivo = "neutro"
    settimane_cand = 0; regime_cand = None
    pers = PERSISTENZA
    ...
    def apri_nuova(cap_new, data, regime, conf, trigger, reg=None):
        ...
        ver_attiva = {
            ...
            "performance_pct": 0.0, "performance_eur": 0.0,
            "giorni_attivo": 0, "composizione": nuova_comp,
            "etf_usciti": [], "aggiornato": data, "_equity": [cap_new],
        }
        versioni.append(ver_attiva)
    ...
    for data_corrente in all_dates:
        ...
        for etf in ver_attiva["composizione"]:
            ...
            etf["price_now"] = round(price_now, 4); etf["perf_pct"] = round(tr, 3)
            etf["perf_eur"]  = round(etf["importo"]*tr/100, 2); etf["drawdown_pct"] = dd
            dd_soglia = {"M": -10, "O": -3, "A": -25}.get(linea_id, -20)
            if dd <= dd_soglia and not trigger_dd:
                trigger_dd = {"ticker": t, "dd": dd, "motivo": f"{t} drawdown {dd:.1f}%"}
            tot_v += etf["importo"] * (1 + tr/100)
        ...
        ver_attiva["performance_pct"]  = round(perf_eur/ver_attiva["capitale_inizio"]*100, 2)
        ver_attiva["aggiornato"] = data_corrente
        ver_attiva.setdefault("_equity", []).append(round(tot_v, 2))
        equity_curve.append(round(tot_v, 2)); equity_dates.append(data_corrente)
        try:
            giorni_check = (datetime.date.fromisoformat(data_corrente) -
                            datetime.date.fromisoformat(ultimo_check)).days
        except Exception:
            giorni_check = 0
        cambio_regime = None; rotation = None
        if giorni_check >= 5:
            ultimo_check = data_corrente
            if linea_id != "M":
                sc, conf = classify_regime_at_date(proxy_data, data_corrente)
                if sc != regime_attivo:
                    ...
                    if (settimane_cand >= pers["settimane"] and
                        ...
                        cambio_regime = {"da": regime_attivo, "a": sc, "confidence": conf}
                        regime_attivo = sc; settimane_cand = 0; regime_cand = None
                else:
                    settimane_cand = 0; regime_cand = None
            if not trigger_dd and not cambio_regime and linea_id == "A":
                rotation = check_rotation(ver_attiva["composizione"], pool, etf_data,
                                          linea_id, data_corrente, giorni,
                                          soglia_rot, regime_attivo)
        cap_new = ver_attiva["capitale_attuale"]
        if trigger_dd:
            t_out = trigger_dd["ticker"]
            etf_out = next((e for e in ver_attiva["composizione"] if e["ticker"]==t_out), None)
            if etf_out:
                ver_attiva["composizione"] = [e for e in ver_attiva["composizione"] if e["ticker"]!=t_out]
                ver_attiva["etf_usciti"].append({
                    "ticker":t_out,"motivo":"drawdown","data":data_corrente,
                    "price_entry":etf_out["price_entry"],
                    "price_exit":etf_out.get("price_now",etf_out["price_entry"]),
                    "perf_pct":etf_out.get("perf_pct",0),"perf_eur":etf_out.get("perf_eur",0),
                ...
                for t in pool:
                    ...
                    if sc_sost > 10: candidati_sost.append((t, sc_sost, pr))
                if candidati_sost:
            ...
            etf_out_obj = next((e for e in ver_attiva["composizione"] if e["ticker"]==t_out), None)
            ...
            if etf_out_obj and pr_in:
                ver_attiva["etf_usciti"].append({
                    ...
                    "mom3m_out":rotation["mom3m_out"],"mom3m_in":rotation["mom3m_in"],
                    "replaced_by":t_in,
                })
                imp_libero = etf_out_obj["importo"]
                ver_attiva["composizione"] = [e for e in ver_attiva["composizione"] if e["ticker"]!=t_out]
                sig_in = etf_data.get(t_in, {})
                cl_in  = [sig_in["closes"][i] for i in range(min(len(sig_in.get("closes",[])),
                          len(sig_in.get("dates",[]))))
                          if sig_in.get("dates",[])[i] <= data_corrente] if sig_in.get("closes") else []
                m3_in  = calc_momentum(cl_in[-64:], min(63, len(cl_in)-1)) if cl_in else 0
    ...
    if len(equity_curve) > 1:
        ...
        for d, v in zip(equity_dates, equity_curve):
            mesi_vals.setdefault(d[:7], []).append(v)
        prev = CAPITALE
        for ym in sorted(mesi_vals.keys()):
            last = mesi_vals[ym][-1]
            rend_mensili[ym] = round((last-prev)/prev*100, 2) if prev else 0
            prev = last
    ...
    for i in range(1, len(versioni)):
        ...
        diario.append({
            "data":v_new.get("data_apertura"), "versione_da":v_old.get("versione"),
            "versione_a":v_new.get("versione"), "trigger":v_new.get("trigger_apertura","—"),
    ...
    n_ver = len(versioni); n_reb = n_ver-1
    cap_fin  = ver_attiva.get("capitale_attuale", CAPITALE)
    perf_fin = round((cap_fin-CAPITALE)/CAPITALE*100, 2)
    n_rot = sum(len([u for u in v.get("etf_usciti",[]) if u.get("motivo")=="rotation_momentum"])
                for v in versioni)
    vol_str = f", vol {vol_tot:.1f}%, MDD {mdd_tot:.1f}%, Sharpe {sharpe_tot:.2f}" if vol_tot else ""
    print(f"      → {n_ver} ver, {n_reb} rib, {n_rot} rot, perf {perf_fin:+.1f}%{vol_str}")

    # Equity curve mensile — coerente con rendimenti_mensili
    equity_mensile = []
    val = float(CAPITALE)
    for ym in sorted(rend_mensili.keys()):
        val = round(val * (1 + rend_mensili[ym] / 100), 2)
        equity_mensile.append({"mese": ym, "valore": val})

    # Punti chiave versioni — apertura/chiusura con capitale esatto
    punti_versioni = []
    for v in versioni:
        punti_versioni.append({
            "versione": v.get("versione"),
            "data_apertura": v.get("data_apertura"),
            "data_chiusura": v.get("data_chiusura"),
            "capitale_inizio": v.get("capitale_inizio"),
            "capitale_fine": v.get("capitale_attuale"),
            "perf_pct": v.get("performance_pct"),
            "regime": v.get("regime"),
            "trigger": v.get("trigger_apertura"),
            "n_etf": len(v.get("composizione", [])),
        })
    ...
    return {
        ...
        "punti_versioni": punti_versioni,
        "diario_movimenti": diario,
        "versione_corrente": n_ver, "benchmark": info["benchmark"],
        "rotation_suggerita": None,
    }

# ── MAIN ───────────────────────────────────────────────────────────────────

# FORECAST REGIME — presente + futuro prossimo
# ══════════════════════════════════════════════════════════════════════

# ── REGOLE HARD ────────────────────────────────────────────────────────────
# Ogni regola: (label, descrizione, condizione_fn, contributi)
# contributi = {scenario: punti}  — applicati se condizione vera
# Peso 2x rispetto alle regole soft

FORECAST_REGOLE_HARD = [
    {
        "id": "R1",
        "label": "VXX spike forte",
        "desc": "VXX accelera > +8% — panico in ingresso",
        "proxy": "VXX",
        "op": ">", "soglia": 8.0,
        "contributi": {"risk_off": 30},
    },
    {
        "id": "R2",
        "label": "VXX spike moderato",
        "desc": "VXX accelera > +4% — stress in aumento",
        "proxy": "VXX",
        "op": ">", "soglia": 4.0,
        "contributi": {"risk_off": 15},
    },
    {
        "id": "R3",
        "label": "Risk-on forte",
        "desc": "SPY-TLT delta > +6% — equity accelera su bond",
        "proxy": "SPY-TLT",
        "op": ">", "soglia": 6.0,
        "contributi": {"goldilocks": 25, "euforia": 15},
    },
    {
        "id": "R4",
        "label": "Flight-to-quality forte",
        "desc": "SPY-TLT delta < -6% — rotazione massiva verso bond",
        "proxy": "SPY-TLT",
        "op": "<", "soglia": -6.0,
        "contributi": {"risk_off": 25},
    },
    {
        "id": "R5",
        "label": "Flight-to-quality moderato",
        "desc": "SPY-TLT delta < -3% — preferenza bond in aumento",
        "proxy": "SPY-TLT",
        "op": "<", "soglia": -3.0,
        "contributi": {"risk_off": 15},
    },
    {
        "id": "R6",
        "label": "Credit spread deteriora",
        "desc": "HYG-LQD delta < -2% — stress creditizio in aumento",
        "proxy": "HYG-LQD",
        "op": "<", "soglia": -2.0,
        "contributi": {"risk_off": 20, "stagflazione": 10},
    },
    {
        "id": "R7",
        "label": "Credit spread migliora",
        "desc": "HYG-LQD delta > +2% — appetito rischio in recupero",
        "proxy": "HYG-LQD",
        "op": ">", "soglia": 2.0,
        "contributi": {"goldilocks": 15, "euforia": 10},
    },
    {
        "id": "R8",
        "label": "Gold accelera forte",
        "desc": "GLD delta > +4% — inflazione o flight-to-safety",
        "proxy": "GLD",
        "op": ">", "soglia": 4.0,
        "contributi": {"reflazione": 20, "risk_off": 10},
    },
    {
        "id": "R9",
        "label": "Gold accelera moderato",
        "desc": "GLD delta > +2% — pressione inflattiva emergente",
        "proxy": "GLD",
        "op": ">", "soglia": 2.0,
        "contributi": {"reflazione": 10},
    },
    {
        "id": "R10",
        "label": "Petrolio accelera forte",
        "desc": "USO delta > +6% — reflazione o shock offerta",
        "proxy": "USO",
        "op": ">", "soglia": 6.0,
        "contributi": {"reflazione": 25, "stagflazione": 15},
    },
    {
        "id": "R11",
        "label": "Petrolio crolla",
        "desc": "USO delta < -6% — domanda in calo, risk-off",
        "proxy": "USO",
        "op": "<", "soglia": -6.0,
        "contributi": {"risk_off": 10, "neutro": 10},
    },
    {
        "id": "R12",
        "label": "TIPS accelera su nominali",
        "desc": "TIP-TLT delta > +2% — inflazione attesa in salita",
        "proxy": "TIP-TLT",
        "op": ">", "soglia": 2.0,
        "contributi": {"reflazione": 20, "stagflazione": 10},
    },
    {
        "id": "R13",
        "label": "Nominali accelerano su TIPS",
        "desc": "TIP-TLT delta < -2% — inflazione attesa in calo",
        "proxy": "TIP-TLT",
        "op": "<", "soglia": -2.0,
        "contributi": {"goldilocks": 15, "risk_off": 5},
    },
    {
        "id": "R14",
        "label": "EM outperforma USA",
        "desc": "EEM-SPY delta > +3% — reflazione globale, dollaro debole",
        "proxy": "EEM-SPY",
        "op": ">", "soglia": 3.0,
        "contributi": {"reflazione": 20},
    },
    {
        "id": "R15",
        "label": "USA outperforma EM",
        "desc": "EEM-SPY delta < -4% — risk-off, fuga dai mercati emergenti",
        "proxy": "EEM-SPY",
        "op": "<", "soglia": -4.0,
        "contributi": {"risk_off": 15, "stagflazione": 5},
    },
    {
        "id": "R16",
        "label": "Dollaro accelera",
        "desc": "UUP delta > +3% — stress globale, flight to USD",
        "proxy": "UUP",
        "op": ">", "soglia": 3.0,
        "contributi": {"risk_off": 15, "stagflazione": 10},
    },
    {
        "id": "R17",
        "label": "Dollaro si indebolisce",
        "desc": "UUP delta < -2% — reflazione, risk-on globale",
        "proxy": "UUP",
        "op": "<", "soglia": -2.0,
        "contributi": {"reflazione": 15, "goldilocks": 5},
    },
    # ── EUROPA / WORLD / FX ───────────────────────────────────────────
    {
        "id": "R18",
        "label": "Europa batte USA",
        "desc": "EZU-SPY delta > +3% — risk-on europeo, divergenza positiva",
        "proxy": "EZU-SPY",
        "op": ">", "soglia": 3.0,
        "contributi": {"goldilocks": 20, "reflazione": 10},
    },
    {
        "id": "R19",
        "label": "USA rifugio vs Europa",
        "desc": "EZU-SPY delta < -4% — stress sistemico o shock EU",
        "proxy": "EZU-SPY",
        "op": "<", "soglia": -4.0,
        "contributi": {"risk_off": 15, "stagflazione": 5},
    },
    {
        "id": "R20",
        "label": "World ex-US batte USA",
        "desc": "ACWX-SPY delta > +3% — dollaro debole, reflazione globale",
        "proxy": "ACWX-SPY",
        "op": ">", "soglia": 3.0,
        "contributi": {"reflazione": 20, "goldilocks": 10},
    },
    {
        "id": "R21",
        "label": "USA domina World ex-US",
        "desc": "ACWX-SPY delta < -4% — fuga verso safe haven americano",
        "proxy": "ACWX-SPY",
        "op": "<", "soglia": -4.0,
        "contributi": {"risk_off": 20, "stagflazione": 5},
    },
    {
        "id": "R22",
        "label": "World intero batte USA",
        "desc": "ACWI-SPY delta > +3% — risk-on globale diffuso",
        "proxy": "ACWI-SPY",
        "op": ">", "soglia": 3.0,
        "contributi": {"reflazione": 15, "goldilocks": 10},
    },
    {
        "id": "R23",
        "label": "USA batte World intero",
        "desc": "ACWI-SPY delta < -4% — concentrazione nel safe haven USA",
        "proxy": "ACWI-SPY",
        "op": "<", "soglia": -4.0,
        "contributi": {"risk_off": 15},
    },
    {
        "id": "R24",
        "label": "Euro forte — risk-on EU",
        "desc": "EURUSD delta > +2% — economia EU in ripresa, risk-on globale",
        "proxy": "EURUSD=X",
        "op": ">", "soglia": 2.0,
        "contributi": {"reflazione": 15, "goldilocks": 10},
    },
    {
        "id": "R25",
        "label": "Dollaro forte — stress globale",
        "desc": "EURUSD delta < -2% — fuga verso USD, stress globale",
        "proxy": "EURUSD=X",
        "op": "<", "soglia": -2.0,
        "contributi": {"risk_off": 10, "stagflazione": 10},
    },
]

# Soglie segnale forecast
FORECAST_SOGLIE = {
    "HOLD":   {"prob_min": 0,   "stesso_regime": True},
    "WATCH":  {"prob_min": 55,  "stesso_regime": False},
    "ROTATE": {"prob_min": 65,  "stesso_regime": False},
}


def forecast_regime(proxy_data, target_date, regime_attuale):
    """
    Calcola il regime previsto a 4 settimane con segnale HOLD/WATCH/ROTATE.

    Metodo:
    1. Strato SOFT: delta momentum (rendimento 2W recenti - rendimento 2W precedenti)
       per tutti i 15 proxy × matrice ETF_PROXY → score direzionale per scenario
    2. Strato HARD: 17 regole esplicite sui differenziali chiave (peso 2x)
    3. Normalizzazione → probabilità per scenario
    4. Segnale: HOLD / WATCH / ROTATE → regime_4w

    Returns: dict completo con scores, segnale, regole attivate, drivers
    """
    import datetime
    ...
    # ── Helper: rendimento su finestra ────────────────────────────────
    def get_ret_window(ticker, date_end, days_start, days_end):
        """Rendimento da days_end a days_start giorni fa (es. 11-22 = settimane 3-4)"""
        d = proxy_data.get(ticker)
        if not d: return None
        closes = d.get("closes", [])
        dates  = d.get("dates", [])
        n = min(len(closes), len(dates))
        cl = [closes[i] for i in range(n) if dates[i] <= date_end]
        if len(cl) < days_end + 1: return None
        p_end   = cl[-(days_start + 1)] if len(cl) > days_start else cl[0]
        p_start = cl[-(days_end + 1)]
        if not p_start: return None
        return (p_end - p_start) / p_start * 100

    # ── Strato SOFT: delta momentum ───────────────────────────────────
    # mom_recente = rendimento ultime 2 settimane (10 giorni)
    # mom_passato = rendimento settimane 3-4 (giorni 11-22)
    # delta = mom_recente - mom_passato
    deltas = {}
    for ticker in ETF_PROXY:
        mom_rec  = get_ret_window(ticker, target_date, 0,  10)
        mom_pass = get_ret_window(ticker, target_date, 10, 22)
        if mom_rec is None or mom_pass is None:
            deltas[ticker] = None
            continue
        delta = mom_rec - mom_pass
        deltas[ticker] = round(delta, 3)

        # Contributo soft: delta × peso_scenario × 10
        for sc in SCENARI:
            peso_sc = ETF_PROXY[ticker].get(sc, 0)
            scores[sc] += delta * peso_sc * 10

        if abs(delta) > 2:
            drivers.append({
                "proxy": ticker,
                "delta": round(delta, 2),
                "direzione": "↑" if delta > 0 else "↓",
            })

    # ── Strato HARD: regole esplicite (peso 2x) ────────────────────────
    # Calcola delta per i proxy compositi
    def get_delta_composito(expr):
        """Es. 'SPY-TLT' → delta(SPY) - delta(TLT)"""
        if '-' in expr:
            parts = expr.split('-')
            d1 = deltas.get(parts[0])
            d2 = deltas.get(parts[1])
            if d1 is None or d2 is None: return None
            return d1 - d2
        return deltas.get(expr)

    for regola in FORECAST_REGOLE_HARD:
        delta_val = get_delta_composito(regola["proxy"])
        if delta_val is None:
            regola["attivata"] = False
            regola["delta_val"] = None
            continue

        # Valuta condizione
        if regola["op"] == ">":
            attivata = delta_val > regola["soglia"]
        else:
            attivata = delta_val < regola["soglia"]

        regola["delta_val"] = round(delta_val, 2)
        regola["attivata"]  = attivata

        if attivata:
            # Applica contributi × 2 (peso doppio)
            for sc, pts in regola["contributi"].items():
                scores[sc] += pts * 2
            regole_attivate.append({
                "id":    regola["id"],
                "label": regola["label"],
                "desc":  regola["desc"],
                "proxy": regola["proxy"],
                "delta": round(delta_val, 2),
                "contributi": regola["contributi"],
            })

    # ── Normalizzazione → probabilità ────────────────────────────────
    # Shift positivo per evitare negativi
    ...
    # Regime forecast e probabilità
    regime_4w  = max(probs, key=probs.get)
    prob_4w    = probs[regime_4w]

    # Regime secondario (secondo più probabile)
    sorted_probs = sorted(probs.items(), key=lambda x: x[1], reverse=True)
    regime_2nd = sorted_probs[1][0] if len(sorted_probs) > 1 else None
    prob_2nd   = sorted_probs[1][1] if len(sorted_probs) > 1 else 0

    # ── Segnale ───────────────────────────────────────────────────────
    stesso_regime = regime_4w == regime_attuale
    if stesso_regime or prob_4w < 55:
        segnale = "HOLD"
        segnale_emoji = "✅"
        segnale_desc  = f"Regime {regime_attuale} stabile — nessun cambio previsto a 4 settimane"
    elif prob_4w < 65:
        segnale = "WATCH"
        segnale_emoji = "⚠️"
        segnale_desc  = f"Possibile transizione verso {regime_4w} ({prob_4w:.0f}%) — monitorare"
    else:
        segnale = "ROTATE"
        segnale_emoji = "🔴"
        segnale_desc  = f"Rotazione verso {regime_4w} prevista ({prob_4w:.0f}%) — prepararsi al ribilancio"

    # Top 3 drivers per delta assoluto
    top_drivers = sorted(drivers, key=lambda x: abs(x["delta"]), reverse=True)[:3]

    return {
        "data":            target_date,
        "regime_attuale":  regime_attuale,
        "regime_4w":       regime_4w,
        "prob_4w":         prob_4w,
        "regime_2nd":      regime_2nd,
        "prob_2nd":        prob_2nd,
        "probabilita":     probs,
        "segnale":         segnale,
        "segnale_emoji":   segnale_emoji,
        "segnale_desc":    segnale_desc,
        "regole_attivate": regole_attivate,
        "n_regole_attivate": len(regole_attivate),
        ...
        "regole_complete": [
            {
                ...
                "op":       r["op"],
                "soglia":   r["soglia"],
                "contributi": r["contributi"],
                "attivata": r.get("attivata", False),
                "delta_val": r.get("delta_val"),
            }
            for r in FORECAST_REGOLE_HARD
        ],
    }



def genera_commento_forecast(forecast, regime_attuale, conf_attuale):
    """
    Genera un commento narrativo in italiano sul forecast di regime.
    Gestisce esplicitamente i conflitti tra driver.
    
    Returns: dict con commento_presente, commento_futuro, conflitti, tono
    """
    segnale     = forecast.get("segnale", "HOLD")
    regime_4w   = forecast.get("regime_4w", regime_attuale)
    prob_4w     = forecast.get("prob_4w", 0)
    regime_2nd  = forecast.get("regime_2nd", "")
    prob_2nd    = forecast.get("prob_2nd", 0)
    attivate    = forecast.get("regole_attivate", [])
    drivers     = forecast.get("top_drivers", [])
    probs       = forecast.get("probabilita", {})

    # ── Classificazione dei driver attivati ──────────────────────────
    # Raggruppa per direzione macro
    risk_on_rules  = {"R3","R7","R13","R18","R20","R22","R24"}  # goldilocks/euforia/EU-world risk-on
    risk_off_rules = {"R1","R2","R4","R5","R6","R15","R16","R19","R21","R23","R25"}  # stress/USD forte
    inflaz_rules   = {"R8","R9","R10","R12","R14"}
    deflaz_rules   = {"R11","R13"}

    ids_att = {r["id"] for r in attivate}
    risk_on_att  = ids_att & risk_on_rules
    risk_off_att = ids_att & risk_off_rules
    inflaz_att   = ids_att & inflaz_rules
    deflaz_att   = ids_att & deflaz_rules

    n_risk_on  = len(risk_on_att)
    n_risk_off = len(risk_off_att)
    n_inflaz   = len(inflaz_att)
    conflitto  = n_risk_on > 0 and n_risk_off > 0

    # ── Commento sul PRESENTE ─────────────────────────────────────────
    reg_labels = {
        "goldilocks":  "Goldilocks — crescita senza inflazione",
        "euforia":     "Euforia — momentum azionario dominante",
        "reflazione":  "Reflazione — crescita con inflazione in salita",
        "stagflazione":"Stagflazione — inflazione senza crescita",
        "risk_off":    "Risk-Off — fuga dal rischio",
        "neutro":      "Neutro — segnali misti",
    }
    reg_desc = reg_labels.get(regime_attuale, regime_attuale)

    conf_txt = "alta" if conf_attuale >= 65 else "moderata" if conf_attuale >= 50 else "bassa"

    if not attivate:
        commento_presente = (
            f"Il regime corrente è {reg_desc} con confidence {conf_txt} ({conf_attuale}%). "
            f"I 15 proxy di mercato non mostrano accelerazioni significative in nessuna direzione: "
            f"il mercato è in una fase di equilibrio. "
            f"Nessuna delle 17 regole hard è attivata — il momentum è diffuso ma non direzionale."
        )
    else:
        # Costruisce lista driver leggibile
        driver_strs = []
        for r in attivate[:4]:
            delta = r.get("delta", 0)
            direzione = "in accelerazione" if delta > 0 else "in decelerazione"
            driver_strs.append(f"{r['proxy']} {direzione} ({delta:+.1f}%): {r['desc']}")

        driver_txt = " · ".join(driver_strs)

        if conflitto:
            commento_presente = (
                f"Il regime corrente è {reg_desc} ({conf_attuale}% confidence), "
                f"ma i proxy mostrano segnali contrastanti. "
                f"Da un lato {len(risk_on_att)} regola/e segnalano risk-on "
                f"({', '.join(risk_on_att)}), dall'altro {len(risk_off_att)} segnalano "
                f"stress ({', '.join(risk_off_att)}). "
                f"Driver attivi: {driver_txt}. "
                f"Situazione di transizione — lettura meno affidabile del solito."
            )
        else:
            if n_risk_off > n_risk_on and n_risk_off >= 2:
                tensione = "Lo stress sui mercati è in aumento"
            elif n_risk_on > n_risk_off and n_risk_on >= 2:
                tensione = "Il risk appetite è in espansione"
            elif n_inflaz >= 2:
                tensione = "Le pressioni inflattive stanno accelerando"
            else:
                tensione = "Il segnale è direzionale ma non ancora estremo"

            commento_presente = (
                f"Il regime corrente è {reg_desc} con confidence {conf_txt} ({conf_attuale}%). "
                f"{tensione}. "
                f"{len(attivate)} delle 17 regole hard sono attivate: {driver_txt}."
            )

    # ── Commento sul FUTURO 4 settimane ──────────────────────────────
    stesso = regime_4w == regime_attuale

    if segnale == "HOLD":
        if conf_attuale >= 65:
            commento_futuro = (
                f"A 4 settimane il modello prevede continuità del regime {regime_attuale} "
                f"({prob_4w:.0f}% di probabilità). "
                f"Il momentum dei proxy non mostra segnali di inversione imminente. "
                f"Linea O e allocazione Fase 2 rimangono invariate."
            )
        else:
            commento_futuro = (
                f"A 4 settimane il regime dovrebbe restare {regime_attuale}, "
                f"ma la confidence corrente è {conf_txt} ({conf_attuale}%) — "
                f"margine di incertezza elevato. "
                f"Lo scenario alternativo più probabile è {regime_2nd} ({prob_2nd:.0f}%). "
                f"Monitorare i driver nelle prossime 2 settimane."
            )

    elif segnale == "WATCH":
        impatto_O = {
            "goldilocks":  "la Linea O potrebbe spostarsi verso composizione goldilocks (HY + IG corporate)",
            "euforia":     "la Linea O potrebbe ridurre la duration e aumentare HY",
            "reflazione":  "la Linea O potrebbe aumentare l'esposizione a EM bond e USD HY",
            "stagflazione":"la Linea O dovrebbe spostarsi verso gov breve e ridurre HY",
            "risk_off":    "la Linea O dovrebbe migrare verso gov breve (IBGS, XGSH) — massima difesa",
            "neutro":      "la Linea O manterrebbe composizione neutro — bilanciata",
        }
        imp = impatto_O.get(regime_4w, "la composizione della Linea O potrebbe cambiare")

        commento_futuro = (
            f"A 4 settimane si intravede una possibile transizione verso {regime_4w} "
            f"({prob_4w:.0f}% di probabilità). "
            f"Il segnale è WATCH: non ancora sufficiente per agire, ma {imp}. "
            f"Se il segnale si consolida nelle prossime 2 settimane, "
            f"la Fase 2 potrebbe raccomandare un ribilancio. "
            f"Scenario alternativo: {regime_2nd} ({prob_2nd:.0f}%)."
        )

    else:  # ROTATE
        impatto_A = {
            "goldilocks":  "La Linea A mantiene il momentum — regime favorevole all'azionario",
            "euforia":     "La Linea A è nel suo regime ideale — massimo momentum",
            "reflazione":  "La Linea A potrebbe ruotare verso commodity e EM",
            "stagflazione":"La Linea A dovrebbe ridurre l'esposizione azionaria — regime avverso",
            "risk_off":    "La Linea A subirà pressione — considerare riduzione in Fase 2",
            "neutro":      "La Linea A mantiene posizione ma monitora i segnali",
        }
        impatto_O = {
            "goldilocks":  "Linea O verso HY e corporate IG",
            "euforia":     "Linea O verso HY — rendimento massimo",
            "reflazione":  "Linea O verso EM bond e USD HY hedged",
            "stagflazione":"Linea O verso gov breve — protezione capital",
            "risk_off":    "Linea O verso IBGS e XGSH — massima difesa",
            "neutro":      "Linea O composizione neutro",
        }
        imp_a = impatto_A.get(regime_4w, "")
        imp_o = impatto_O.get(regime_4w, "")

        commento_futuro = (
            f"ROTATE verso {regime_4w} ({prob_4w:.0f}% di probabilità) — "
            f"segnale forte confermato da {len(attivate)} regole. "
            f"{imp_a}. {imp_o}. "
            f"La Fase 2 suggerirà un ribilancio M/O/A al prossimo run se il segnale persiste. "
            f"Probabilità residua su {regime_2nd}: {prob_2nd:.0f}% — "
            f"{'da non ignorare' if prob_2nd >= 20 else 'minoritario'}."
        )

    # ── Conflitti ─────────────────────────────────────────────────────
    note_conflitti = []
    if conflitto:
        note_conflitti.append(
            f"⚠ Segnali contrastanti: {len(risk_on_att)} regole risk-on vs "
            f"{len(risk_off_att)} regole risk-off — affidabilità del forecast ridotta"
        )
    if prob_4w < 55 and segnale != "HOLD":
        note_conflitti.append(
            f"⚠ Probabilità 4W ({prob_4w:.0f}%) sotto la soglia di confidenza — "
            f"segnale borderline"
        )
    if prob_2nd >= 30:
        note_conflitti.append(
            f"⚠ Scenario alternativo {regime_2nd} al {prob_2nd:.0f}% — "
            f"dispersione alta tra gli scenari"
        )

    # ── Tono generale ─────────────────────────────────────────────────
    if segnale == "ROTATE" and regime_4w in ("risk_off", "stagflazione"):
        tono = "difensivo"
    elif segnale == "ROTATE" and regime_4w in ("goldilocks", "euforia"):
        tono = "costruttivo"
    elif segnale == "WATCH":
        tono = "vigile"
    elif conflitto:
    ...
    return {
        "commento_presente": commento_presente,
        "commento_futuro":   commento_futuro,
        "conflitti":         note_conflitti,
        "tono":              tono,
        "n_driver_risk_on":  n_risk_on,
        "n_driver_risk_off": n_risk_off,
        "n_driver_inflaz":   n_inflaz,
        "conflitto_rilevato": conflitto,
    }


def main():
    oggi = datetime.date.today().isoformat()
    print(f"COMPASS 3 Linee v2.1 — {oggi}")
    print(f"Linee: {LINEE} · Capitale: €{CAPITALE:,} per linea · Fase 2: ATTIVA")
    ...
    if OUT_FILE.exists():
        try:
            ...
            if existing.get("version","") != "3linee_2.0":
                print(f"  Versione precedente — forza backtest completo")
                existing = {}
            else:
    ...
    # ── 1. Proxy ──────────────────────────────────────────────────────
    print(f"\n[1/6] Download {len(ETF_PROXY)} ETF proxy...")
    proxy_data = {}
    for t in ETF_PROXY:
    ...
    print(f"\n[2/6] Download {len(tutti_ticker)} ETF universo...")
    etf_data = {}
    success = 0; errors = 0
    for i, ticker in enumerate(sorted(tutti_ticker)):
        ...
        if not raw:
            ...
            etf_data[ticker] = {
                "ticker": ticker, "closes": raw["closes"], "dates": raw.get("dates",[]),
                "yield_pct": raw["yield_pct"], "price": round(raw["current_price"], 4),
            }
            success += 1
            print(f"OK score={calc_score(raw['closes'], 'O', raw['yield_pct'])}")
        time.sleep(0.35)
    print(f"  Download: {success} OK, {errors} ERR")
    for t, d in proxy_data.items():
        if d and t not in etf_data: etf_data[t] = d

    # ── 3. Backtest ───────────────────────────────────────────────────
    print(f"\n[3/6] Backtest 3 linee (da {BACKTEST_START})...")
    portafogli = {}
    FORCE_RESET = {"O"}
    ...
    for linea_id in LINEE:
        if linea_id in portafogli_esistenti and linea_id not in FORCE_RESET:
            ...
            if ver_att:
                ...
                ver_att["performance_pct"]  = round((tot_v-ver_att["capitale_inizio"])/ver_att["capitale_inizio"]*100,2)
                ver_att["aggiornato"] = oggi
                info = LINEE_INFO[linea_id]
                rot = check_rotation(ver_att["composizione"], info["pool"], etf_data,
                                     linea_id, oggi,
                ...
                # Ricalcola equity_mensile e punti_versioni dai dati esistenti
                # equity_mensile: da rendimenti_mensili già nel JSON
                rend_mens = portafogli[linea_id].get("rendimenti_mensili", {})
                eq_mens = []
                val_eq = float(CAPITALE)
                for ym in sorted(rend_mens.keys()):
                    val_eq = round(val_eq * (1 + rend_mens[ym] / 100), 2)
                    eq_mens.append({"mese": ym, "valore": val_eq})
                # Aggiunge mese corrente con valore aggiornato
                mese_oggi = oggi[:7]
                if eq_mens and eq_mens[-1]["mese"] < mese_oggi:
                    eq_mens.append({"mese": mese_oggi, "valore": round(cap_fin, 2)})
                portafogli[linea_id]["equity_mensile"] = eq_mens

                # punti_versioni: da storia già nel JSON
                pv = []
                for v in storia:
                    pv.append({
                        "versione":      v.get("versione"),
                ...
                portafogli[linea_id]["punti_versioni"] = pv
    ...
    for linea_id, info in LINEE_INFO.items():
        ...
        if not sig or not sig.get("closes") or not sig.get("dates"):
            benchmarks[bm] = {"ticker":bm,"perf_pct":None}; continue
        ...
        print(f"  {bm}: {'+' if perf and perf>=0 else ''}{perf:.1f}%" if perf else f"  {bm}: N/D")
    ...
    # ── 5. Forecast regime ───────────────────────────────────────────
    print(f"\n[5/6] Forecast regime (presente + futuro prossimo)...")
    risultato_forecast = forecast_regime(proxy_data, oggi, sc_oggi)
    commento_forecast  = genera_commento_forecast(risultato_forecast, sc_oggi, conf_oggi)
    risultato_forecast["commento"] = commento_forecast
    segnale = risultato_forecast["segnale"]
    print(f"  Regime attuale : {sc_oggi} ({conf_oggi}%)")
    print(f"  Regime 4W      : {risultato_forecast['regime_4w']} ({risultato_forecast['prob_4w']:.0f}%)")
    print(f"  Segnale        : {risultato_forecast['segnale_emoji']} {segnale} — {risultato_forecast['segnale_desc']}")
    print(f"  Tono           : {commento_forecast['tono']}")
    print(f"  Regole attivate: {risultato_forecast['n_regole_attivate']}/17")
    if commento_forecast['conflitto_rilevato']:
        print(f"  ⚠ Conflitto driver: risk-on vs risk-off")
    if risultato_forecast['top_drivers']:
        drivers_str = ' · '.join(f"{d['proxy']} {d['direzione']}{abs(d['delta']):.1f}%" for d in risultato_forecast['top_drivers'])
        print(f"  Driver: {drivers_str}")

    # ── 6. Fase 2 ─────────────────────────────────────────────────────
    print(f"\n[6/6] Fase 2 — Rotation capitale tra le 3 linee...")
    ...
    print(f"  Livello crisi : {risultato_fase2['livello_crisi']} (score {risultato_fase2['score_crisi']:.0f})")
    print(f"  Allocaz target: M={alloc['M']}%  O={alloc['O']}%  A={alloc['A']}%")
    if rib:
        print(f"  ⚡ RIBILANCIO: M {mov['M']:+,.0f}€ | O {mov['O']:+,.0f}€ | A {mov['A']:+,.0f}€")
        print(f"     Motivo: {risultato_fase2['motivo']}")
    else:
        print(f"  ✓ Nessun ribilancio (delta < {risultato_fase2['soglia_ribilancio_pct']}%)")
    if risultato_fase2["in_rientro"]:
        print(f"  ↩ Rientro sett {risultato_fase2['settimane_rientro']} da {risultato_fase2['data_fine_crisi']}")
    ...
    for t, d in etf_data.items():
        ...
        etf_scores[t] = {
            "ticker":t,"nome":ETF_NOMI.get(t,t),"price":d["price"],
            "mom1m":m1,"mom3m":m3,"yield_pct":d.get("yield_pct",0),
    ...
    composizione_ideale = {
        "O": {
            "regime": sc_oggi, "composizione": comp_O_ideale,
    ...
    output = {
        ...
        "strategy": "3 Linee v2.1 — Monetario / Obbligazionario / Azionario",
        "fase": "2 — rotation capitale attiva",
        "backtest_start": BACKTEST_START,
        "regime_oggi": {"scenario": sc_oggi, "confidence": conf_oggi},
        "range_regime": RANGE_REGIME,
        "forecast": risultato_forecast,
        "fase2": risultato_fase2,
        "composizione_O_per_regime": {
            sc: [{"ticker":e["ticker"],"peso":e["peso"],
                  "yield_stimato":YIELD_STIMATI_O.get(e["ticker"],0)}
                 for e in etf_list]
    ...
    for lid, ptf in portafogli.items():
        ...
        if perf is not None and out is not None:
            print(f"   {flag} Linea {lid} ({LINEE_INFO[lid]['nome']}): {perf:+.1f}% | vs {bm}: {out:+.1f}pp")
        else:
            print(f"   Linea {lid}: N/D")
    print(f"\n   Fase 2: {risultato_fase2['livello_crisi'].upper()} | "
          f"M={alloc['M']}% O={alloc['O']}% A={alloc['A']}%"
          + (" | ⚡ RIBILANCIO" if rib else ""))
