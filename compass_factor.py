...
ETF_CATEGORIA = {
    ...
    "WOEE.DE":"az_globale",  # World Enhanced Active
    "EMEE.MI":"az_em",       # EM Enhanced Active
    "USEE.DE":"az_usa",      # US Enhanced Active
    "EUEE.DE":"az_europa",   # Europe Enhanced Active
    "IART.DE":"tematico",    # AI Innovation (peso max 15%)
    "AXEE.MI":"az_em",       # Asia ex Japan Enhanced Active
    "IFSW.MI":"az_globale",  # STOXX World Multifactor
    "EUEB.MI":"obbligaz_ig", # EUR Corp Bond Enhanced Active
}
...
ETF_TIPO = {
    ...
    # iShares Enhanced Active — tutti Acc
    "WOEE.DE":"Acc","EMEE.MI":"Acc","USEE.DE":"Acc","EUEE.DE":"Acc",
    "IART.DE":"Acc","AXEE.MI":"Acc","IFSW.MI":"Acc","EUEB.MI":"Acc",
}
...
ETF_NOMI = {
    ...
    "VAGE.DE":"Vanguard EUR Aggregate Bond","XEON.MI":"Xtrackers II EUR Overnight Rate Swap",
    ...
    "IFSW.MI":"iShares STOXX World Equity Multifactor",
}
...
def calc_momentum(closes, days):
    if days < 0 or len(closes) < days+1: return None
    old = closes[-(days+1)]
...
def seleziona_etf_factor(pool, etf_data_completo, level_id, alloc_t,
    ...
    # Vincolo peso massimo per ETF tematici aggressivi (solo C9/A9)
    ...
    # Cap peso per ETF vincolati (IART.DE max 15% — tematico aggressivo)
    for tk, max_p in PESO_MAX_VINCOLATO.items():
        if tk in pesi_raw and pesi_raw[tk] > max_p:
            eccesso = pesi_raw[tk] - max_p
            pesi_raw[tk] = max_p
            # Redistribuisce l'eccesso proporzionalmente agli altri
            altri = {t: p for t, p in pesi_raw.items() if t != tk}
            tot_altri = sum(altri.values()) or 1
            for t in altri:
                pesi_raw[t] += eccesso * pesi_raw[t] / tot_altri
...
def main():
    ...
    n_charts = 0
    for ticker in sorted(etf_data.keys()):
        if etf_data[ticker].get("closes"):
            if salva_chart_json(ticker, etf_data, charts_dir):
                n_charts += 1
