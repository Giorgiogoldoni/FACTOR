...
UNIVERSE = [
    ...
    {"t":"WPXJ.MI", "n":"WT Japan EUR Hedged",             "cat":"az_em",      "sub":"JAPAN",     "fonte":"RAPTOR"},
    {"t":"XCHA.MI", "n":"iShares China",                   "cat":"az_em",      "sub":"CHINA",     "fonte":"RAPTOR"},
    {"t":"XASX.MI", "n":"iShares Asia Pacific",            "cat":"az_em",      "sub":"ASIA_PAC",  "fonte":"RAPTOR"},
    ...
    {"t":"EUHA.MI", "n":"PIMCO Euro HY Acc",               "cat":"hy",         "sub":"EU_HY_A",   "fonte":"RAPTOR"},
...
def run_backtest_etp(etf_data, proxy_data, backtest_start, oggi):
    ...
    for idx, rdate in enumerate(rebal_dates):
        ...
        for etf in UNIVERSE:
            ...
            # Vincolo leva: solo in euforia/goldilocks
            if etf["cat"] == "leva" and regime_oggi not in ("goldilocks","euforia"):
                score_finale *= 0.1

            # Vincolo forecast: blocca leva in risk_off/stagflazione forecast
            if etf["cat"] == "leva" and fc["regime_4w"] in ("risk_off","stagflazione") and fc["segnale"] == "ROTATE":
                score_finale = 0
...
def main():
    ...
    print(f"  Performance: {risultato['performance_totale_pct']:+.1f}% | "
          ...
          f"Sharpe: {risultato['sharpe']:.2f if risultato['sharpe'] else '—'}")
    print(f"  Rebalancing eseguiti: {risultato['n_rebalancing']}")

    # ── 4. Benchmark ───────────────────────────────────────────────────────
    print(f"\n[4/4] Benchmark {BENCHMARK}...")
    bm_data = etf_data.get(BENCHMARK)
    bm_perf = None
    if bm_data:
        p_start = get_price_on_date(bm_data["closes"], bm_data["dates"], BACKTEST_START)
        p_end   = bm_data["closes"][-1]
        if p_start:
            bm_perf = round((p_end-p_start)/p_start*100, 2)
            print(f"  {BENCHMARK} da {BACKTEST_START}: {bm_perf:+.1f}%")
            outperf = round(risultato["performance_totale_pct"] - bm_perf, 2)
            print(f"  Outperformance: {outperf:+.1f}pp")
