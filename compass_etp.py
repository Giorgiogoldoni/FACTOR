#!/usr/bin/env python3
"""
COMPASS ETP — compass_etp.py v2.1
══════════════════════════════════════════════════════════════════════
...
def applica_momentum_relativo(candidati, iwmo_mom3m, regime_oggi):
    ...
    for c in candidati:
        ...
        # Bonus/malus proporzionale: ±8% dello score per ogni 10pp di differenza
        fattore = 1.0 + (delta / 10.0) * 0.08
        fattore = max(0.7, min(1.3, fattore))  # clamp [0.7, 1.3]
...
def main():
    ...
    output = {
        ...
        "strategy":     "COMPASS ETP v2.1 — Rel.Momentum (soft) + Concentrazione + Soglia Rotazione",
